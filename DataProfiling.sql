...
/* ---------------------------------------------------------------------
   1. XPONENT_WEEKLY
   --------------------------------------------------------------------- */
SELECT TOP 10 * FROM RAW.XPONENT_WEEKLY;
...
SELECT DISTINCT prescriber_flag FROM RAW.HCP_MASTER; --one HCP who is not prescriber
...
-- 4a. Grain check
SELECT claim_id, COUNT(*) AS n FROM RAW.CLAIMS GROUP BY 1 HAVING COUNT(*) > 1;
...
SELECT * FROM RAW.CLAIMS WHERE refill_number < 0;
...
-- 5a. Grain check
SELECT call_id, COUNT(*) AS n FROM RAW.VEEVA_CALLS GROUP BY 1 HAVING COUNT(*) > 1;
