/* =====================================================================
   ...
   Surrogate keys use HASH() on the natural key rather than an
   ...
   - dim_territory holds geography/org hierarchy ONLY. Rep is its own
     ...
     because rep-to-territory assignment changes over time — integrating it
...
/* ---------------------------------------------------------------------
   ...
   Keep common date attributes here so the fact tables can use the same
   calendar logic for reporting by week, month, quarter, etc.
   --------------------------------------------------------------------- */
...
/* ---------------------------------------------------------------------
   ...
   This dimension contains the geography hierarchy. Rep assignments are
   handled separately because those relationships can change over time.
   --------------------------------------------------------------------- */
...
-- (e.g. a manager change mid-period) — needs a "most recent/current flag" tiebreak



/* ---------------------------------------------------------------------
   MAP_TERRITORY_REP
   Grain: 1 row per territory + rep + effective period. 
   This is the piece that used to be flattened into dim_territory. Kept as its own
...
/* ---------------------------------------------------------------------
   DIM_HCP
   Grain: 1 row per HCP (npi_clean). 
   Carries territory_key ONLY —
...
/* ---------------------------------------------------------------------
   ...
   GOVERNANCE NOTE for future: patient_id is patient-level health data even if
   de-identified upstream. This table should sit behind a restricted
   role / row-access or masking policy in Snowflake — shouldn't be granted
